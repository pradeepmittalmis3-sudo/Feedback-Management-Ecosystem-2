...
CREATE OR REPLACE VIEW public.feedback_dashboard_normalized_v
WITH (security_invoker = true)
AS
WITH unioned_rows AS (
  SELECT
    'migration_data'::text AS source_table,
  ...
  SELECT
    'working_data'::text AS source_table,
    to_jsonb(wd) AS row_json,
    wd.ctid::text AS row_pointer
  FROM public.working_data wd
),
mapped AS (
  SELECT
    source_table,
    ...
    COALESCE(
      ...
      NULLIF(trim(row_json ->> 'id'), ''),
      concat(source_table, '::', row_pointer)
    ) AS record_id,
    COALESCE(
    ...
    public.feedback_parse_boolean(
      COALESCE(
        row_json ->> 'price_challenge_ok',
        row_json ->> 'Price Challenge',
        row_json ->> 'Price challenge'
      )
    ) AS price_challenge,
    public.feedback_parse_boolean(
      COALESCE(
        row_json ->> 'bill_received',
        row_json ->> 'Bill Received'
      )
    ) AS bill_received,
...
deduped AS (
  SELECT
    normalized.*,
    ROW_NUMBER() OVER (
      PARTITION BY normalized.record_id
      ORDER BY normalized.updated_at DESC, CASE WHEN normalized.source_table = 'working_data' THEN 0 ELSE 1 END
    ) AS rn
  FROM normalized
)
SELECT
  ...
  ROUND(
    (
      COALESCE(staff_behavior, 0)
      + COALESCE(staff_service, 0)
      + COALESCE(satisfaction_level, 0)
    )::numeric
    /
    NULLIF(
      (CASE WHEN staff_behavior IS NOT NULL THEN 1 ELSE 0 END)
      + (CASE WHEN staff_service IS NOT NULL THEN 1 ELSE 0 END)
      + (CASE WHEN satisfaction_level IS NOT NULL THEN 1 ELSE 0 END),
      0
    ),
    2
  ) AS overall_rating
...
GRANT SELECT ON public.feedback_dashboard_normalized_v TO anon, authenticated;
...
WITH scope_rows AS MATERIALIZED (
  SELECT *
  FROM public.feedback_dashboard_normalized_v v
...
SELECT jsonb_build_object(
  'summary', (
    SELECT jsonb_build_object(
      ...
      'migration_data_entries', COUNT(*) FILTER (WHERE source_table = 'migration_data'),
      'working_data_entries', COUNT(*) FILTER (WHERE source_table = 'working_data')
    )
    FROM base
  ),
